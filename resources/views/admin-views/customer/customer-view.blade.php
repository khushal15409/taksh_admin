...
@section('title', isset($tab_name) ? $tab_name . ' - ' . translate('Customer Details') : translate('Customer Details'))
            ...
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title mb-1">
                        @if(isset($tab_name))
                            {{ $tab_name }} - {{translate('messages.customer_id')}} #{{$customer['id']}}
                        @else
                            {{translate('messages.customer_id')}} #{{$customer['id']}}
                        @endif
                    </h1>
                    <span class="fs-12">
                    ...
                    <div class="card-body">
                        <div class="d-flex flex-wrap gap-3">
                            ...
                            <div class="color-card flex-column align-items-center justify-content-center color-5 flex-grow-1">
                                ...
                                <div class="d-flex flex-column align-items-center">
                                    <h2 class="title"> {{ \App\CentralLogics\Helpers::format_currency($total_order_amount[0]->total_order_amount ?? 0) }} </h2>
                                    <div class="subtitle">
                            ...
                            <div class="media gap-3 flex-wrap">
                                ...
                                <div class="media-body">
                                    <div class="key-value-list d-flex flex-column gap-2 text-dark" style="--min-width: 60px">
                                        ...
                                        <div class="key-val-list-item d-flex gap-3">
                                            <div>{{ translate('messages.joined_at') }}</div>:
                                            <div class="font-semibold">{{date('d M Y '.config('timeformat'),strtotime($customer['created_at']))}}</div>
                                        </div>
                                        @if($customer->addresses && count($customer->addresses) > 0)
                                            @foreach($customer->addresses as $address)
                                                <div class="key-val-list-item d-flex gap-3">
                                                    <div>{{ translate('address') }} ({{translate($address['address_type'] ?? 'address')}})</div>:
                                                    <a href="https://www.google.com/maps/search/?api=1&query={{ data_get($address,'latitude',0)}},{{ data_get($address,'longitude',0)}}" target="_blank" class="text-dark font-semibold">{{ $address['address'] ?? translate('messages.N/A') }}</a>
                                                </div>
                                            @endforeach
                                        @else
                                            <div class="key-val-list-item d-flex gap-3">
                                                <div>{{ translate('address') }}</div>:
                                                <div class="font-semibold">{{ translate('messages.N/A') }}</div>
                                            </div>
                                        @endif
                                    </div>
