...
class CustomerController extends Controller
{
    ...
    public function view(Request $request,$id)
    {
        ...
        $tab = $request['tab']; // Get tab parameter directly
        $customer = User::with('addresses')->find($id);
        if (isset($customer)) {
            // Use tab parameter if provided, otherwise map from status
            $tab_name = $tab ?? $this->getFMTabNameFromStatus($status);
            ...
            $total_order_amount = Order::selectRaw('sum(order_amount) as total_order_amount')->latest()->where(['user_id' => $id])
                ...
                ->Notpos()->get();
            $orders = Order::withcount('details')->with('store')->latest()->where(['user_id' => $id])
            ->when(isset($key), function($query) use($key){
                $query->Where('id', 'like', "%{$key}%");
            } )
            ->when(isset($status), function($query) use($status){
            ...
            return view('admin-views.customer.customer-view', compact('customer', 'orders','total_order_amount', 'tab_name'));
        }
        Toastr::error(translate('messages.customer_not_found'));
        return back();
    }
    
    private function getFMTabNameFromStatus($status)
    {
        // FM ORDER tab mapping based on status (fallback if tab not provided)
        $statusToTabMap = [
            'pending' => 'OP', // Order Placement
            'accepted' => 'OA', // Vendor order accept
            'processing' => 'OFP', // Out for pickup
            'confirmed' => 'OFP', // Out for pickup
            'picked_up' => 'OP', // Order picked (OP2 in FM ORDER)
            'handover' => 'OS', // Order Shipped
            'canceled' => 'OC', // Order Cancelled
            'refund_requested' => 'OR', // Order Return
        ];
        
        return $statusToTabMap[$status] ?? null;
    }
