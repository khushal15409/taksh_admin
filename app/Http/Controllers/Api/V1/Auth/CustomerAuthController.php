...
class CustomerAuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        // Assign ecommerce-user role using Spatie
        ...
        if(isset($login_settings['phone_verification_status']) && $login_settings['phone_verification_status'] == 1){
            ...
            if(!$firebase_otp_verification){
                ...
                // Static OTP for testing - commented out dynamic OTP generation
                // $otp = rand(100000, 999999);
                // if(env('APP_MODE') == 'test'){
                //     $otp = '123456';
                // }
                $otp = '123456'; // Static OTP for now
                ...
                // Commented out SMS sending function for now
                // $published_status = 0;
    ...
    {
        // First find the user to exclude from unique validation
        ...
        // Get user ID to exclude from unique validation
        // This allows updating the same user with the same details without "already exists" errors
        $userId = $user->id;
        
        $rules = [
            'name' => 'required',
            'login_type' => 'required|in:otp,social,manual',
            'phone' => 'required|min:9|max:14|unique:users,phone,' . $userId,
    ...
    private function send_otp($request_data){
        ...
        return response()->json([
            'code' => 200,
            'status' => true,
            'message' => 'OTP sent successfully on this mobile number'
        ], 200);
    }
