...
class CustomerController extends Controller
{
    ...
    public function validate_30_minute_address(Request $request)
    {
        $validator = Validator::make($request->all(), [
        ...
        // Check if address is in a valid zone
        $zone = Zone::whereContains('coordinates', new Point($latitude, $longitude, POINT_SRID))->first();
        
        if (!$zone) {
            return response()->json([
                'is_available' => false,
                'message' => '30-minute delivery not available in this area',
                'zone_id' => null,
        ...
        // Find stores within 30-minute delivery range
        ...
        $available_stores_count = $stores->count();
        $is_available = $available_stores_count > 0;
        ...
        if ($stores->count() > 0) {
            ...
            $nearest_store = [
                ...
                'estimated_delivery_time' => (int) ($nearest->estimated_delivery_time ?? 30)
            ];
        }

        $estimated_delivery_time = $nearest_store ? $nearest_store['estimated_delivery_time'] : 30;

        return response()->json([
            'type' => 'store',
            'is_available' => $is_available,
            'estimated_delivery_time' => $estimated_delivery_time,
            'message' => $is_available ? '30-minute delivery available' : '30-minute delivery not available in this area',
            'zone_id' => $zone->id,
            'available_stores_count' => $available_stores_count,
