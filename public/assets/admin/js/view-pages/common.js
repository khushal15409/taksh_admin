...
(function() {
    ...
    // Store all immediate jQuery calls to execute after jQuery loads
    var immediateJQueryCalls = [];
    
    // Function to execute all immediate jQuery calls
    function executeImmediateCalls() {
        immediateJQueryCalls.forEach(function(fn) {
            try {
                fn();
            } catch(e) {
                console.error('Error executing jQuery call:', e);
            }
        });
    }
    
    // Wait for jQuery then execute all calls
    waitForJQuery(executeImmediateCalls);
    
    // Export function to add immediate calls - ensure it's available globally
    if (typeof window !== 'undefined') {
        window.addImmediateJQueryCall = function(fn) {
            if (typeof jQuery !== 'undefined' && typeof $ !== 'undefined') {
                fn();
            } else {
                immediateJQueryCalls.push(fn);
            }
        };
    }
})();
...
window.addImmediateJQueryCall(function() {
